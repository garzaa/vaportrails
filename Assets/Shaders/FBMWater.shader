    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            ColorResult fbmChain(in float2 uv) {
                ...
                float2 r = float2( 
                  fbm( uv + 4.0*q + 5*sin(_Time/8.0)),
                  fbm( uv + 4.0*q));

                float t = fbm( uv + 4.0*(r-(_Time/8.0)));
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                if (compareColor(color, _MaskColor, 0.1)) {
                  ...
                  float4 fbmColor = lerp(
                    lerp(col1, col2, round(length(r.q))),
                    lerp(col3, col4, round(r.r.x)),
                    round(r.t)
                  );
